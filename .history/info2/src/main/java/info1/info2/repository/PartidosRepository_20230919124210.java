...
public interface PartidosRepository extends JpaRepository<Partidos, Long> {
    ...
    @Query("SELECT p FROM Partido p WHERE p.equipoLocal.nombre = :nombreEquipo OR p.equipoVisitante.nombre = :nombreEquipo")
    List<Partidos> findByNombreEquipo(@Param("nombreEquipo") String nombreEquipo);

    @Query("SELECT p FROM Partido p WHERE p.fecha = :fecha")
